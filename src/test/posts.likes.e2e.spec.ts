...
describe('Likes Posts (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        appSettings(app);
        const dataSource = app.resolve(DataSource);
        manager = (await dataSource).manager.createQueryBuilder();
        await app.init();
    ...
    it('CreateUser', async () => {
        user1 = (await request(app.getHttpServer()).post('/sa/users').set(headers).send(createUser1).expect(201)).body;

        user2 = (await request(app.getHttpServer()).post('/sa/users').set(headers).send(createUser2).expect(201)).body;
        user3 = (await request(app.getHttpServer()).post('/sa/users').set(headers).send(createUser3).expect(201)).body;
        user4 = (await request(app.getHttpServer()).post('/sa/users').set(headers).send(createUser4).expect(201)).body;
    });
    ...
    it('Likes Posts ', async () => {
        const likesPostUser1 = await request(app.getHttpServer())
        ...
        const res2 = await request(app.getHttpServer()).get(`/posts/${postId}`).expect(200);
        ...
        const likesPostUser3 = await request(app.getHttpServer())
            .put(`/posts/${postId}/like-status`)
            .set({ authorization: 'Bearer ' + token3 })
            .send({ likeStatus: 'Dislike' })
            .expect(204);

        const res3 = await request(app.getHttpServer()).get(`/posts/${postId}`).expect(200);
        ...
        const res4 = await request(app.getHttpServer()).get(`/posts/${postId}`).expect(200);
        ...
        //         .expect(204);
        //
        //     const res6 = await request(app.getHttpServer()).get(`/posts/${postId}`).expect(200);
        //
        ...
        //     const allPostsForSpBlog = await request(app.getHttpServer())
        //         .get(`/blogs/${createPost.blogId}/posts`)
        //         .set({ authorization: 'Bearer ' + token3 })
        //         .expect(200);
        //
